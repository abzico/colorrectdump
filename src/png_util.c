...
void crd_free_png_image_data(png_bytepp data, int height)
{
...
png_bytepp crd_allocate_png_rgba_image_space(int width, int height)
{
	// for 4 channels
...
png_bytepp crd_read_png_file(const char* file_name, int* rst_rowbytes, int* rst_width, int* rst_height)
{
  // open file
  ...
  {
    CRD_ELOG("read %s file error", file_name)
  ...
  {
    // it's not PNG file
    CRD_ELOG("%s file is not recognized as png file\n", file_name)
  ...
  if (png_ptr == NULL)
  {
    ...
    CRD_ELOG("cannot create png structure\n")
  ...
  if (info_ptr == NULL)
  {
    CRD_ELOG("cannot create png info structure\n")
...
#define PNG_READ_SETJMP(png_ptr, info_ptr, fp) \
  ...
  { \
    CRD_ELOG("error png's set jmp for read\n") \
                                              \
    /* clear png resource */                  \
  ...
  CRD_LOG("[PNG] width = %d\n", width)
  CRD_LOG("[PNG] height = %d\n", height)
  CRD_LOG("[PNG] bit_depth = %d\n", bit_depth)
  switch (color_type)
  {
    case PNG_COLOR_TYPE_GRAY:
      CRD_LOG("[PNG] color type = 'PNG_COLOR_TYPE_GRAY' (bit depths 1, 2, 4, 8, 16)\n")
      break;
    case PNG_COLOR_TYPE_GRAY_ALPHA:
      CRD_LOG("[PNG] color type = 'PNG_COLOR_TYPE_GRAY_ALPHA' (bit depths 8, 16)\n")
      break;
    case PNG_COLOR_TYPE_PALETTE:
      CRD_LOG("[PNG] color type = 'PNG_COLOR_TYPE_PALETTE' (bit depths 1, 2, 4, 8)\n")
      break;
    case PNG_COLOR_TYPE_RGB:
      CRD_LOG("[PNG] color type = 'PNG_COLOR_TYPE_RGB' (bit depths 8, 16)\n")
      break;
    case PNG_COLOR_TYPE_RGB_ALPHA:
      CRD_LOG("[PNG] color type = 'PNG_COLOR_TYPE_RGB_ALPHA' (bit depths 8, 16)\n")
      break;
  }
  switch (interlace_type)
  {
    case PNG_INTERLACE_NONE:
      CRD_LOG("[PNG] interlace type = none\n")
      break;
    case PNG_INTERLACE_ADAM7:
      CRD_LOG("[PNG] interlace type = ADAM7\n")
      break;
  }
  switch (channels)
  {
    case 1:
      CRD_LOG("[PNG] channels = %d (GRAY, PALETTE)\n", channels)
      break;
    case 2:
      CRD_LOG("[PNG] channels = %d (GRAY_ALPHA)\n", channels)
      break;
    case 3:
      CRD_LOG("[PNG] channels = %d (RGB)\n", channels)
      break;
    case 4:
      CRD_LOG("[PNG] channels = %d (RGB_ALPHA or RGB + filter byte)\n", channels)
      break;
  }
  CRD_LOG("[PNG] rowbytes = %d\n", rowbytes)
  ...
  png_read_image(png_ptr, row_ptr);
	...
	if (channels == 1)
	{
		CRD_LOG("[PNG] image has %d channels, we need to convert to 4 channels\n", channels)		
		png_bytepp convt_row_ptr = crd_allocate_png_rgba_image_space(width, height);
		...
		crd_free_png_image_data(row_ptr, height);
...
bool crd_write_png_file(const char* file_name, const png_bytepp data, int width, int height)
{ 
  // open file
  // note: libpng tells us to make sure we open in binary mode
...
#define PNG_WRITE_SETJMP(png_ptr, info_ptr, fp) \
  ...
  if (setjmp(png_jmpbuf(png_ptr)))  \
  { \
    CRD_ELOG("error png's set jmp for write\n") \
                                              \
    /* clear png resource */                  \
